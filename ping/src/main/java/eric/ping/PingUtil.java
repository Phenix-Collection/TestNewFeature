...
public final class PingUtil {
     ...
     */
    public static float getMinRTT(String url) {
     ...
     */
    public static float getAvgRTT(String url) {
     ...
     */
    public static float getMaxRTT(String url) {
     ...
     */
    public static float getMdevRTT(String url) {
     ...
     * @return 最小RTT值，单位 ms 注意：-1是默认值，返回-1表示获取失败
     */
    public static float getMinRTT(String url, int count, int timeout) {
        return getRtt(url, count, timeout, 0);
    }
     ...
     * @return 平均RTT值，单位 ms 注意：-1是默认值，返回-1表示获取失败
     */
    public static float getAvgRTT(String url, int count, int timeout) {
        return getRtt(url, count, timeout, 1);
    }
     ...
     * @return 最大RTT值，单位 ms 注意：-1是默认值，返回-1表示获取失败
     */
    public static float getMaxRTT(String url, int count, int timeout) {
        return getRtt(url, count, timeout, 2);
    }
     ...
     * @return RTT平均偏差，单位 ms 注意：-1是默认值，返回-1表示获取失败
     */
    public static float getMdevRTT(String url, int count, int timeout) {
        return getRtt(url, count, timeout, 3);
    }
     ...
     * @param count
     * @param timeout
     * @param index 0:minRTT, 1:avgRTT, 2:maxRTT, 3:mdevRTT
     * @return -1为失败
     */
    private static float getRtt(String ip, int count, int timeout, @IntRange(from = 0, to = 3) int index) {
        ...
        String pingString = ping(createSimplePingCommand(count, timeout, ip));
        if (null != pingString) {
            System.out.println("get" + getType(index) + "RTT result: " + pingString);
            try {
                String tempInfo = pingString.substring(pingString.indexOf("min/avg/max/mdev") + 19);
                String[] results = tempInfo.split("\\s+", 2);
                String[] temps = results[0].split("/");
                System.out.println("rtt:" + Arrays.toString(temps));
                return Float.valueOf(temps[index]);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
